...
test("World data files are generated for given worlds and saved to blob with correct name", async () => {
    ...
    const fetchWorldList = () =>
        Promise.resolve([createWorldStatus("en01"), createWorldStatus("en02")])
...
test("World full info blob is generated with all data files", async () => {
    const fetchWorldList = () =>
        Promise.resolve([createWorldStatus("en01"), createWorldStatus("en123")])
    const worldDataDates = ["2022_08_22", "2022_08_21", "2022_08_20", "2022_07_10"]
    const getWorldDataFileNames = (worldCode: string) =>
        Promise.resolve(worldDataDates.map(date => `${worldCode}/data/${date}.json`))
