...
export const generateDataForWorlds = async (
    ...
    await Promise.all(
        worldList.map(async worldStatus => {
            if (!worldStatus.isClosed) {
                ...
                const fileName = `${currentDate.getUTCFullYear()}_${String(
                    currentDate.getUTCMonth() + 1
                ).padStart(2, "0")}_${String(currentDate.getUTCDate()).padStart(2, "0")}`
                ...
                for (const ocean of oceans) {
                    ...
                    if (existingOceanFiles.includes(`${worldStatus.id}/ocean/${fileName}.png`))
                        continue
            ...
            const WorldFullInfo = {
                ...worldStatus,
                avialableDates: savedDates
                    .map(date => date.match(/(\w+)(.json)$/)[0])
...
const calculateAbsoluteTownPositions = (towns: GrepolisTown[], islands: Island[]): BlobTown[] =>
    towns
        .map(town => {
            ...
            const islandOffset = islandPositionOffsetMatrix
                .filter(islandOffset => islandOffset.islandType === island.islandType)
                .find(islandOffset => islandOffset.posOnIsland === town.posOnIsland)
            ...
            const size =
                TOWN_MIN_SIZE + (TOWN_MAX_SIZE - TOWN_MIN_SIZE) * (town.points / TOWN_MAX_POINTS)
...
const generateOceanImage = async (
    ...
    const islandsInOcean = islands.filter(
        i =>
            i.x > islandBoundMinX &&
            i.x < islandBoundMaxX &&
            i.y > islandBoundMinY &&
...
const getIslandImage = async (
    ...
    islandImage.resize(
        islandImage.bitmap.width * ISLAND_SCALE,
        islandImage.bitmap.height * ISLAND_SCALE
    )
    return islandImage
}
